...
import {
  ...
  Lightbulb, TrendingUp, TrendingDown, Calendar, MapPin, Globe,
  BookOpen, Users, Target, ArrowRight, FileText, Save, Wrench,
...
const trends = [
  { title: "AI-Powered Climate Modeling for Africa", direction: "up" as const, change: "+34%", region: "Sub-Saharan Africa", explanation: "Growing interest in localized climate models using machine learning for agricultural planning and disaster preparedness across the continent.", tags: ["AI", "Climate", "Agriculture"] },
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="bg-card rounded-xl border border-border p-4 w-full lg:w-64 space-y-2">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange}>
          <TabsList className="bg-secondary">
            <TabsTrigger value="journals" className="gap-1"><BookOpen className="h-3 w-3" /> Journals</TabsTrigger>
            <TabsTrigger value="conferences" className="gap-1"><Calendar className="h-3 w-3" /> Conferences</TabsTrigger>
            <TabsTrigger value="stakeholders" className="gap-1"><Users className="h-3 w-3" /> Stakeholders</TabsTrigger>
            <TabsTrigger value="trends" className="gap-1"><TrendingUp className="h-3 w-3" /> Trends</TabsTrigger>
            <TabsTrigger value="gaps" className="gap-1"><Target className="h-3 w-3" /> Gaps</TabsTrigger>
          </TabsList>
                ...
                <div key={j.name} className="bg-card rounded-xl border border-border p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 hover:shadow-sm transition-shadow">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      ...
                      {", "}
                      {j.openAccess ? <span className="text-primary">Open Access</span> : <span>Peer-reviewed</span>}
                      {", "}
                      {j.deadline}
                    </p>
                  </div>
              ...
              {conferences.map((c) => (
                <div key={c.name} className="bg-card rounded-xl border border-border p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                        ...
                        <div className="space-y-3 text-sm">
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button variant="outline" size="sm" className="gap-1 text-xs" onClick={() => {
                              toast({ title: "Reminder set", description: `You'll be notified before the ${c.name} deadline.` });
                            }}>🔔 Remind Me</Button>
                          </div>
                    ...
                    <Button
                      variant={savedConferences.includes(c.name) ? "secondary" : "outline"}
                      size="sm"
                      className="text-xs"
                      onClick={() => saveConference(c.name)}
                    >
                  ...
                  <div className="flex gap-2">
                    <Dialog>
                      ...
                      <DialogContent>
                        ...
                        <Button variant="afrika" size="sm" className="w-full mt-2" onClick={() => toast({ title: "Request sent", description: `Connection request sent to ${s.name}.` })}>
                          Send Request
                        </Button>
                      </DialogContent>
                    </Dialog>
              ...
              {trends.map((t) => (
                <div key={t.title} className="bg-card rounded-xl border border-border p-5 space-y-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <Button
                      variant={savedTrends.includes(t.title) ? "secondary" : "outline"}
                      size="sm"
                      className="text-xs gap-1"
                      onClick={() => saveTrend(t.title)}
                    >
                    ...
                    <Link to={`/dashboard/instrument-studio?prompt=${encodeURIComponent(`Create a research instrument to measure trends in: ${t.title}`)}`}>
                      <Button variant="afrikaOutline" size="sm" className="text-xs gap-1"><Wrench className="h-3 w-3" /> Create Instrument</Button>
                    </Link>
                  </div>
                </div>
              ))}
              ...
              {gaps.map((g) => (
                <div key={g.title} className="bg-card rounded-xl border border-border p-5 space-y-3">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <Button
                      variant={savedGaps.includes(g.title) ? "secondary" : "ghost"}
                      size="sm"
                      className="w-full gap-1 text-xs"
                      onClick={() => saveGap(g.title)}
                    >
